...
const createHistory = async (userId, residentId, description) => {
  try {
    await pool.execute(
      'INSERT INTO history (user_id, resident_id, description) VALUES (?, ?, ?)',
      [userId, residentId, description]
    );
    return true;
  } catch (error) {
...
const getRecentHistory = async (req, res) => {
  try {
    ...
    const [history] = await pool.execute(
      `SELECT h.id, h.user_id, h.resident_id, h.description, h.timestamp,
